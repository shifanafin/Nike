...
import {setRemoveItemsFromCart,setincrement,setdecrement,setClearCart} from '../../app/CartSlice.js'
...
const CartItems = ({item:{id,text,title,img,color,shadow,price,cartQuality}}) => {
  ...
  const removeItems =()=>{
    ...
    const quantityRemove = ()=>{
      dispatch(setdecrement({id,text,title,img,color,shadow,price,cartQuality}))
    }
    const onClearAction = ()=>{
      dispatch(setClearCart({id,text,title,img,color,shadow,price,cartQuality}))
    }
  return (
